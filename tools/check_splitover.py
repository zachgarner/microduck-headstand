...
from eval_checkpoint import contacts, is_standing

p = argparse.ArgumentParser()
p.add_argument("--task", default="Mjlab-HeadstandSplitOver-Flat-MicroDuck")
...
env.event_manager.get_term_cfg("set_roulade_state").params["mirror_prob"] = 1.0 if args.mirrored else 0.0
...
with torch.no_grad():
    for i in range(150):
        ...
        feet_down = contacts(env)["feet"] > 0
        for j in range(N):
            if first_foot[j] is None and win[j] and feet_down[j]:
                first_foot[j] = i * env.step_dt
standing = int(is_standing(env, asset).sum())
